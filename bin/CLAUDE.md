...
When run inside a managed worktree (one with `.worktree-session.json`), the
`[name]` argument is optional — the active session is detected automatically.
...
| `bin/worktree down [name]` | Stop server and remove worktree (self-destructive inside managed worktree) |
